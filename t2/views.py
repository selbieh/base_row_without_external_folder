...
from baserow.contrib.database.api.rows.errors import ERROR_ROW_DOES_NOT_EXIST, ERROR_ROW_IDS_NOT_UNIQUE
...
from t2.serializers import CrunchBaseOrganizationSerializer
...
class CrunchBaseOrganization(APIView):
    ...
    def get_cb_url(self, request, cb_field_value):
        try:
            cb_url= cb_field_value.split('/')[-1]
            if '#' in cb_url:
                cb_url=cb_url.split('/')[1]
            return cb_url
        except :
    ...
    def call_cb(self,request,cb_permalink):
        ...
        url=f'{baseurl}?card_ids=raised_funding_rounds&user_key={settings.CB_KEY}'
    ...
    def map_cb_response_to_request_data(self,cb_call_response,validated_data):
        return {
            validated_data['company_prev_raised_count_field_name']:len(cb_call_response['cards']['raised_funding_rounds']) ,
            validated_data['company_total_raised_value_field_name']: 0 if len(cb_call_response['cards']['raised_funding_rounds'])==0 else sum([item['money_raised']['value_usd'] for item in cb_call_response['cards']['raised_funding_rounds']]),
